...
class HeidelpayCD_Edition_Model_Payment_Abstract extends Mage_Payment_Model_Method_Abstract
{
        ...
        $RefId = false)
    {
        ...
        Mage::dispatchEvent('heidelpay_getHeidelpayUrl_bevor_preparePostData',
            array(
                'order' => $order,
                'config' => $config,
                'frontend' => $frontend,
    ...
    public function getFrontend($ordernr, $storeId = false)
    {
        return array(
            ...
            'FRONTEND.SUCCESS_URL' =>
                Mage::getUrl('hcd/index/success',
                    array(
                        '_forced_secure' => true,
                        '_store_to_url' => true,
                        '_nosid' => true
            ...
            'FRONTEND.FAILURE_URL' =>
                Mage::getUrl('hcd/index/error',
                    array(
                        '_forced_secure' => true,
                        '_store_to_url' => true,
                        '_nosid' => true
            ...
            'CRITERION.LANGUAGE' =>
                strtolower(Mage::helper('hcd/payment')->getLang()),
            'CRITERION.STOREID' =>
        ...
        if ($isReg === false
            ...
            if ($this->getCustomerData($this->_code, $billing->getCustomerId())) {
                ...
                if ($this->_code == 'hcdcc' or $this->_code == 'hcddc') {
                    ...
                    if (
                        isset($paymentData['payment_data']['ACCOUNT_EXPIRY_MONTH'])
                    ) {
