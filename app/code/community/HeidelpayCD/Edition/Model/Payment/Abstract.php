 ...
 *
 */
class HeidelpayCD_Edition_Model_Payment_Abstract
    extends Mage_Payment_Model_Method_Abstract
{
    ...
    public function getHeidelpayUrl($isRecognition=null, $basketId=null, $refId=null)
    {
        ...
        if ($isRecognition === null) {
            ...
            $ordernr = ($customerId == 0)
                ? $visitorData['visitor_id'] : $customerId;
            $order = $this->getQuote();
        }
        ...
        if ($this->_code == 'hcdcc' or $this->_code == 'hcddc') {
            ...
            // set frame to sync modus if frame is used in bevor order mode
            // (this is the registration case)
            $criterion['FRONTEND.PREVENT_ASYNC_REDIRECT'] =
                ($isRecognition === true) ? 'TRUE' : 'FALSE';
        }
        ...
        $user = $this->getUser($order, $isRecognition);


        if ($isRecognition === null) {
            $completeBasket = ($config['INVOICEING'] == 1 or
                $this->_code == "hcdbs") ? true : false;
    ...
    public function getBasketData($order, $completeBasket = false, $amount = false)
    {
        $data = array(
            'PRESENTATION.AMOUNT' =>
                ($amount) ? $amount : Mage::helper('hcd/payment')
    ...
    public function getFrontend($ordernr, $storeId = false)
    {
        return array(
            ...
            'CRITERION.LANGUAGE' => strtolower(
                Mage::helper('hcd/payment')->getLang()
            ),
    ...
    public function getUser($order, $isRecognition = null)
    {
        $user = array();
        ...
        $user['ADDRESS.STREET'] = trim(
            $billing->getStreet1() . " " . $billing->getStreet2()
        );
        ...
        if ($isRecognition === null and $order->getPayment()->getMethodInstance()
                ->activeRedirect() === true
        ) {
            if ($this->getCustomerData($this->_code, $billing->getCustomerId())) {
                $paymentData = $this->getCustomerData(
                    $this->_code, $billing->getCustomerId()
                );


                $this->log(
                    'getUser Customer: ' . print_r($paymentData, 1), 'DEBUG'
                );
                ...
                if ($this->_code == 'hcdcc' or $this->_code == 'hcddc') {
                    if (isset(
                        $paymentData['payment_data']['ACCOUNT_BRAND']
                    )) {
                        unset($paymentData['payment_data']['ACCOUNT_BRAND']);
                    }

                    if (isset(
                        $paymentData['payment_data']['ACCOUNT_NUMBER']
                    )) {
                        unset($paymentData['payment_data']['ACCOUNT_NUMBER']);
                    }

                    if (isset(
                        $paymentData['payment_data']['ACCOUNT_HOLDER']
                    )) {
                        unset($paymentData['payment_data']['ACCOUNT_HOLDER']);
                    }

                    if (isset(
                        $paymentData['payment_data']['ACCOUNT_EXPIRY_MONTH']
                    )) {
                        unset($paymentData['payment_data']['ACCOUNT_EXPIRY_MONTH']);
                    }

                    if (isset(
                        $paymentData['payment_data']['ACCOUNT_EXPIRY_YEAR']
                    )) {
    ...
    public function getBasket($order)
    {
        ...
        if ($items) {
            ...
            foreach ($items as $item) {
                ...
                $parameters[$prefix . '.PERCENT_VAT'] =
                    sprintf('%1.2f', $item->getTaxPercent());
        ...
        if ($this->getShippingNetPrice($order) > 0) {
            ...
            $parameters[$prefix . '.PERCENT_VAT'] =
                $this->getShippingTaxPercent($order);
        ...
        if ($order->getDiscountAmount() < 0) {
            ...
            $parameters[$prefix . '.UNIT'] = 'Stk.'; // Liter oder so
            $parameters[$prefix . '.AMOUNT_UNIT_GROSS'] =
                floor(bcmul($order->getDiscountAmount(), 100, 10));
            $parameters[$prefix . '.AMOUNT_GROSS'] =
                floor(bcmul($order->getDiscountAmount(), 100, 10));
    ...
    public function canCapture()
    {
        ...
        if (Mage::getModel('hcd/transaction')
                ->getOneTransactionByMethode(
                    $order->getRealOrderId(), 'PA'
                ) === false
        ) {
            $this->log('there is no preauthorisation for the order '
    ...
    public function capture(Varien_Object $payment, $amount)
    {
        ...
        if ($this->canCapture()) {
            ...
            if ($Autorisation === false) {
                Mage::throwException(Mage::helper('hcd')
                    ->__('This Transaction could not be capture online.'));
                return $this;
            }
    ...
    public function refund(Varien_Object $payment, $amount)
    {
        ...
        $CaptureData = Mage::getModel('hcd/transaction')
            ->loadLastTransactionDataByUniqeId((string)$payment->getRefundTransactionId());
    ...
    public function getCustomerName($session = false)
    {
        if ($session === true) {
            $session = $this->getCheckout();
            return $session->getQuote()->getBillingAddress()
                    ->getFirstname() . ' ' .
    ...
    public function saveCustomerData($data, $uniqeID = null)
    {
        ...
        $custumerData->setPaymentData(
            Mage::getModel('hcd/resource_encryption')->encrypt(json_encode($data))
        );
    ...
    public function getCustomerData($code = false, $customerId = false, $storeId = false)
    {
        ...
        $customerId = ($customerId) ? $customerId :
            $this->getQuote()->getBillingAddress()->getCustomerId();
